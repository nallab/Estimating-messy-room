...
x_train = np.genfromtxt('data/train_features3.csv', dtype=np.float32, delimiter=',')
x_val = np.genfromtxt('data/val_features3.csv', dtype=np.float32, delimiter=',')
y_train = np.genfromtxt('data/train_labels3.csv', dtype=np.uint8)
y_val = np.genfromtxt('data/val_labels3.csv', dtype=np.uint8)
#print(x_train.shape) -> (3840, 2048)
# custom head: one hidden layer.


model = Sequential([
    Dense(10, activation='sigmoid', kernel_regularizer=regularizers.l2(weight_decay)),
    #Dense(10, activation='sigmoid', kernel_regularizer=regularizers.l2(weight_decay),input_shape=(2048,)),
